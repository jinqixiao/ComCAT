...
from comcat_diffusion import inject_trainable_comcat, save_comcat_weight
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=("File path for unet comcat to resume training."),
    )
    parser.add_argument(
        "--resume_text_encoder",
        type=str,
        default=None,
        help=("File path for text encoder comcat to resume training."),
    )
...
def main(args):
    ...
    unet_comcat_params, _ = inject_trainable_comcat(
        unet, args.comcat_rank, target_replace_module='CrossAttention'
    )
    unet.cuda()
    ...
    params_to_optimize = (
        [
            {"params": unet_comcat_params, "lr": args.learning_rate},
        ]
        if args.train_text_encoder
        else unet_comcat_params
    )
            ...
            if accelerator.sync_gradients:
                if args.save_steps and global_step - last_save >= args.save_steps:
                    if accelerator.is_main_process:
                        ...
                        filename_unet = (
                            f"{args.output_dir}/comcat_weight_e{epoch}_s{global_step}.pt"
                        )

                        print(f"save weights {filename_unet}")
                        save_comcat_weight(unet_comcat_params, filename_unet)
