...
class Attention2(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.w_qs = nn.Linear(dim, dim, bias=qkv_bias)
        self.w_ks = nn.Linear(dim, dim, bias=qkv_bias)
        self.w_vs = nn.Linear(dim, dim, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim, bias=qkv_bias)
        self.proj_drop = nn.Dropout(proj_drop)

    def convert2attn2(self, attn):
        w = torch.split(attn.qkv.weight.data, attn.proj.weight.data.shape[0])
        self.w_qs.weight.data = w[0]
        self.w_ks.weight.data = w[1]
        self.w_vs.weight.data = w[2]
        if self.w_qs.bias is not None:
            b = torch.split(attn.qkv.bias.data, attn.proj.weight.data.shape[0])
            self.w_qs.bias.data = b[0]
            self.w_ks.bias.data = b[1]
            self.w_vs.bias.data = b[2]
        self.proj = attn.proj
    ...
    def forward(self, x):
        B, N, C = x.shape
        d = C // self.num_heads
        q = self.w_qs(x).view(B, N, self.num_heads, d).transpose(1, 2)
        k = self.w_ks(x).view(B, N, self.num_heads, d).transpose(1, 2)
        v = self.w_vs(x).view(B, N, self.num_heads, d).transpose(1, 2)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
...
class Attention_newSVD(nn.Module):
    def __init__(self, atten, is_pretrained=True, rank=None, rank_ratio=None, attn2_with_bias=False, attn_drop=0,
                 drop=0):
        super().__init__()
        ...
        if rank_ratio is not None:
            rank = int(self.head_dim / rank_ratio)
            assert (self.head_dim > rank > 0)

        if is_pretrained:
            if isinstance(atten, Attention2):
                wq = atten.w_qs.weight.data.T.reshape(atten.w_qs.in_features, self.num_heads, self.head_dim).transpose(
                    0, 1)
                wk = atten.w_ks.weight.data.T.reshape(atten.w_qs.in_features, self.num_heads,
            ...
            for i in range(self.num_heads):
                if rank is not None:
                    l, r = self.SVDByRank(w_qk[i, :, :], rank)
                    wq_l.append(l)
                    wk_r.append(r)

            wq = torch.cat(wq_l, 1)
            wk = torch.cat(wk_r, 0).T
            self.w_qs = nn.Linear(atten.dim, wq.shape[1], bias=attn2_with_bias)
            self.w_ks = nn.Linear(atten.dim, wk.shape[1], bias=attn2_with_bias)
            ...
            self.w_vs = nn.Linear(atten.dim, wv.shape[1], bias=attn2_with_bias)
            self.attn_drop = nn.Dropout(attn_drop)
            self.proj = nn.Linear(wr.shape[0], atten.dim, bias=attn2_with_bias)
            self.proj_drop = nn.Dropout(drop)

            self.w_vs.weight.data = wv.T
            self.proj.weight.data = wr.T
        else:
            self.w_qs = nn.Linear(atten.w_qs.in_features, int(self.num_heads * rank), bias=attn2_with_bias)
            self.w_ks = nn.Linear(atten.w_ks.in_features, int(self.num_heads * rank), bias=attn2_with_bias)
            self.w_vs = nn.Linear(atten.w_vs.in_features, int(self.num_heads * rank), bias=attn2_with_bias)
            self.attn_drop = nn.Dropout(attn_drop)
            self.proj = nn.Linear(int(self.num_heads * rank), atten.proj.out_features, bias=attn2_with_bias)
            self.proj_drop = nn.Dropout(drop)
        if attn2_with_bias:
    ...
    def SVDByRank(self, dense_w, rank):
        u, s, v = np.linalg.svd(
    ...
    def SVDByPCR(self, dense_w, ratio):
        u, s, v = np.linalg.svd(
            dense_w.detach().squeeze().cpu().numpy(), full_matrices=False)
        ...
        v = np.diag(s) @ v
        right_factor = torch.from_numpy(v)
        return left_factor, right_factor, rank
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        # qkv_bias = False
        # print(dim, num_heads, qkv_bias, attn_drop, drop)
        if search_rank:
            attn = Attention2(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
            qk_rank = None
            vp_rank = None
            if ranks is not None:
                qk_rank = ranks[0]
                vp_rank = ranks[1]
            self.attn = AttentionNewSVD4AutoRank(attn, attn2_with_bias=attn2_with_bias, qk_rank=qk_rank,
                                                 vp_rank=vp_rank)
        elif use_attn2:
        ...
        if new_svd:
            self.attn = Attention_newSVD(self.attn, is_pretrained=False, rank_ratio=attn_compress_ratio,
                                         attn2_with_bias=attn2_with_bias)
            # self.attn = Attention_SVD(self.attn, rank_ratio=attn_compress_ratio)
    ...
    def convert2attn2(self):
        attn2 = Attention2(self.attn.proj.weight.data.shape[0], num_heads=self.attn.num_heads,
                           qkv_bias=(self.attn.qkv.bias is not None), attn_drop=self.attn.attn_drop.p,
                           proj_drop=self.attn.proj_drop.p)
        attn2.convert2attn2(self.attn)
        self.attn = attn2

    def forward(self, x):
        # print((self.attn(self.norm1(x)) - self.attn3(self.norm1(x))).sum())
        # print("x:", x.shape)
        x = x + self.drop_path(self.attn(self.norm1(x)))
        # print("atten :", self.attn)
        # print("x :", x.shape)
        # print("mlp :", self.mlp)
    ...
    def search_rank_forward(self, x, params_to_accumulate):
        # print((self.attn(self.norm1(x)) - self.attn3(self.norm1(x))).sum())
        o = self.norm1(x)
        o, f = self.attn.search_rank_forward(o, params_to_accumulate)
        x = x + self.drop_path(o)
        o = self.norm2(x)
        o, f = self.mlp.search_rank_forward(o, f)
        x = x + self.drop_path(o)
        return x, f
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if search_rank:
            ...
            if ranks is not None:
                rank = ranks[-1]
            self.head = SVDLinear4AutoRank(head, rank)
        else:
    ...
    def search_rank_forward(self, x):
        x = self.patch_embed(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_token, x), dim=1)
        x = self.pos_drop(x + self.pos_embed)
        f = 0
        for block in self.blocks:
            x, f = block.search_rank_forward(x, f)
        x = self.norm(x)
        x = self.pre_logits(x[:, 0])
        x, f = self.head.search_rank_forward(x, f)
        return x, f
...
@register_model
def deit_lite_patch16_224(pretrained=False, **kwargs):
    """
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=87, depth=6, num_heads=3, **kwargs)
    model = _create_vision_transformer('deit_lite_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
